...
module Fastlane
  ...
  module Helper
    class BrowserstackHelper
      ...
      def self.upload_file(browserstack_username, browserstack_access_key, file_path, url, custom_id = nil, ios_keychain_support = true)
        payload = {
          multipart: true,
          file: File.new(file_path, 'rb')
        }

        if ios_keychain_support
            payload[:data] = '{ "ios_keychain_access": "' + ios_keychain_support.to_s + '" }'
        else
